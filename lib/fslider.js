...
wsf.Server = function (server, options) {
  ...
  this._sockets = {};
  this._namespace = [];
  this._namespace.push(options.namespace);
...
wsf.Server.prototype.getNamespace = function () {
  return this._namespace;
};

wsf.Server.prototype.registNamespace = function (namespace) {
...
wsf.Server.prototype.bind = function (server, options) {
  if (server) {
    ...
    if (!this.server) 
      this.httpServer = server, 
      this.registNamespace(options.namespace);
  } else {
    return;
  }
};
...
wsf.Server.prototype.unbind = function () {
  if (this.httpServer && this.httpServer instanceof http.Server)
    this.httpServer = null, this._namespace = [];
};
...
function upgrade_handler(req, socket) {
  ...
  if (server._namespace.indexOf(req.url) == -1) 
    return;
